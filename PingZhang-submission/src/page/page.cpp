...
{
    // TODO
    // return true when the page is not empty and the offset is less than the size of the page.
    if ((bytes.empty() == false) && (offset < bytes.size())){ 
        return true;
    }
    else{
        return false;
...
{
    // TODO
    if (is_valid_offset(offset) == true){  
            return bytes[offset];
    }
    else{
        return 0;
